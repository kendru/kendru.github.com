...
In the last chapter, we looked at ClojureScript's versions of we usually call
...
The first, and perhaps most common, expression that we will study in this chapter
...
#### Get Hypotenuse Length Iteratively
...
#### Generating Product Variations with `for`
...
To use any of these modifiers, we can simply append it to the list of sequence
...
![Behaviour of :while Modifier](/img/learn-cljs/chapter11/while-short-circuit.png)
...
In the next chapter, we will look in more detail at recursive functions - that is,
...
traditional imperative loop. footnote:[Interestingly enough, `loop` compiles
down to a `while` loop in JavaScript]. Translating our `gcd` function from above
into a `loop`/`recur` form is trivial:

#### Implementing `gcd` with `loop`
...
the function. As we mentioned in the last chapter, every time a recursive
...
If you enter this into the REPL, it will print the numbers 0-9, just as we would
...
required. This laziness is, as we will see in a later chapter, very useful for
...
- [Previous: Making Choices](/learn-clojurescript/2019/09/13/making-choices/)